...
export async function generateListingBrochure(listingId) {
    ...
    const { title = "Untitled Listing", address = "", price, downPayment, beds, baths, sqft, aiFullDescription, description, aiTags, imageUrls, } = data || {};
    const mainImageUrl = Array.isArray(imageUrls) && imageUrls.length ? imageUrls[0] : undefined;
    const brochureInput = {
        title,
        address,
...
async function createBrochurePdf(input) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (typeof input.price === "number" && !Number.isNaN(input.price)) {
                priceLine.push(`$${input.price.toLocaleString("en-US", {
                    maximumFractionDigits: 0,
                })}`);
            }
            if (typeof input.beds === "number")
                priceLine.push(`${input.beds} bd`);
            if (typeof input.baths === "number")
                priceLine.push(`${input.baths} ba`);
            if (typeof input.sqft === "number") {
                priceLine.push(`${input.sqft.toLocaleString("en-US", {
                    maximumFractionDigits: 0,
                })} sq ft`);
            }
            if (priceLine.length) {
            ...
            if (typeof input.downPayment === "number") {
                doc
                    ...
                    .fontSize(10)
                    .fillColor("#16a34a")
                    .text(`Approx. $${input.downPayment.toLocaleString("en-US", {
                    maximumFractionDigits: 0,
                })} down (estimate)`);
            }
            ...
            doc
                .fontSize(8)
                .fillColor("#9ca3af")
                .text("This brochure was auto-generated by HI AWTO from listing data and AI-assisted descriptions. Not a legal disclosure or appraisal.", {
                align: "left",
            });
            doc.end();
        }
