...
import {
  ...
  loadAiInsights,
  // OPTIONAL: if you have helpers to fetch raw fraud events + listings
  loadFraudEvents,
...
export default function AdminDashboard() {
  ...
  // Load time-series chart + AI insights when range changes
  useEffect(() => {
    setLoadingSeries(true);

    Promise.all([
      loadListingsTimeSeries(range),
      loadFraudTrendSeries(range),
      ...
      .then(
        ([
          ...
          setFraudEvents(fraudEventsRaw || []);
          setAdminListings(listingsRaw || []);
        }
      )
  ...
  const maxCount =
    series.length > 0 ? Math.max(...series.map((s) => s.count)) || 1 : 1;
  const maxFraud =
    fraudSeries.length > 0
      ? Math.max(...fraudSeries.map((s) => s.score)) || 1
      : 1;
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Track marketplace activity, growth, fraud risk, and health.
          </p>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-slate-100 p-6">
          <DashboardSectionHeader
            ...
            subtitle="Filter by date range or export the data."
            range={range}
            setRange={setRange}
            onExport={() => {
          ...
          <div className="h-48 flex items-end gap-2 border-t border-slate-100 pt-4">
            {loadingSeries ? (
              ...
              series.map((point, idx) => {
                const height =
                  maxCount === 0 ? 10 : 14 + (point.count / maxCount) * 100;
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-slate-100 p-6">
          ...
          <p className="text-xs text-slate-500 mb-3">
            AI-generated risk scores over time.
          </p>
          ...
          <div className="h-48 flex items-end gap-2 border-t border-slate-100 pt-4">
            {fraudSeries.length === 0 ? (
              ...
              fraudSeries.map((p, idx) => {
                const height = 10 + (p.score / maxFraud) * 100;
...
function StatCard({ title, primary, sublabel, detail, accent }) {
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
      ...
      <div className="text-3xl font-bold text-slate-900 mt-1">{primary}</div>
